...
function ProgBar({ pct }: { pct: number }) {
  return (
    <View style={pb.wrap}>
      <View style={pb.row}>
        <Text style={pb.left}>PROGRESO DE PERFIL</Text>
        <Text style={pb.right}>{pct}%</Text>
      </View>
      <View style={pb.track}>
...
export default function OnboardingName() {
  // Precargar con nombre del auth si existe
    ...
    <SafeAreaView style={s.container}>
      <View style={s.content}>
        <TouchableOpacity style={s.backRow} onPress={() => router.back()}>
          ...
          <Text style={s.backTitle}>MetaPAES</Text>
        </TouchableOpacity>
        ...
        <View style={s.card}>
          ...
          <Text style={s.desc}>Queremos personalizar tu experiencia de estudio.</Text>
          ...
          <TouchableOpacity
            ...
            onPress={() => {
              if (!name.trim()) return
              // Pasar el nombre via router params al siguiente paso
              router.push({ pathname: '/(auth)/onboarding/career', params: { name: name.trim() } })
            }}
            activeOpacity={0.85}
          >
        ...
        <Text style={s.step}>Paso 1 de 4 — Configura tu perfil</Text>
      </View>
    </SafeAreaView>
  )
}
...
const s = StyleSheet.create({
  ...
  title: { fontFamily: typography.family.extrabold, fontSize: 26, letterSpacing: -0.02, color: colors.onSurface, marginBottom: 10 },
